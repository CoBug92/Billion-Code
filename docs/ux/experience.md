...
В MVP нет tab bar. Граф — корневой опыт; список людей является частью того же экрана. Каждая строка списка и поисковой выдачи использует портрет человека: сначала локальный, затем удалённый. Пока удалённое изображение загружается или если оно недоступно, показывается персональная монограмма. Профиль, карточка связи, источник и настройки открываются поверх или из navigation stack с предсказуемым возвратом к позиции камеры и выбору.
...
- Дальний план разбивает всех людей на пропорциональные секции по отраслям; это presentation-слой, а не новые domain-сущности или редакционные главы.
- Человек может состоять в нескольких отраслях. В каждой секции показывается его layout-ссылка, но все ссылки открывают один профил и одну историю навигации.
- После приближения сцена остаётся единой сетью без центрального героя.
...
- Компактная легенда находится поверх сцены. Её элементы неинтерактивны и не скрывают типы узлов.
...
- На стартовом масштабе видны все люди и достаточно крупные отраслевые секции. Tap по подписи секции приближает её и включает интерактивные портретные узлы.
- Без selection детальный план показывает только людей: организации, университеты и линии связей появляются как контекст выбранного узла.
- Досье человека показывает все его отрасли в атласе; переход к связанной сущности и возврат назад восстанавливают исходную отраслевую позицию.
...
- Действие «Показать весь граф» возвращает исходные позиции и камеру. Локальный граф также завершается при закрытии selection, переходе из досье или выборе через поиск ноды вне текущего окружения.
...
Имена людей в русском интерфейсе отображаются по-русски. Для известных персон используется русское имя из Wikidata или Wikipedia, для остальных — импортированный транслитерированный вариант, который требует редакторской проверки перед публикацией.

Полное имя подписывается только при достаточном масштабе и наличии портрета. Компании и университеты используют сокращённые названия и могут сохранять официальное латинское написание бренда. Полное имя каждой интерактивной сущности остаётся в карточке и accessibility label; touch target не меньше 44×44 pt.
