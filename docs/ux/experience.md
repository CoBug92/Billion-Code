...
Панель является контекстным «живым досье» плотной сети. Без selection она скрыта и не отнимает место у графа. После выбора человека, компании или университета панель появляется поверх нижней четверти сцены.

Состояния:

- `compact` — около 25% доступной высоты с пределами для малых и больших экранов;
- `expanded` — вся высота окна: панель закрывает сцену, а прокручиваемый контент учитывает safe areas.

Drag назначен header/handle панели, чтобы не конкурировать с прокруткой. При Reduce Motion detent и выбор меняются без пространственной анимации. VoiceOver предоставляет действия «Развернуть» и «Свернуть». В compact-состоянии tap по фону или повторный tap по ноде очищает selection и скрывает панель.

Compact-досье человека показывает полное имя, датированную оценку состояния либо честное отсутствие опубликованной оценки, значимые компании и первый датированный этап пути. Для компании показываются деятельность, основатели и связанные люди; для университета — тип, место, направления и люди графа.

Expanded-досье объединяет факты, связи и единую вертикальную хронологию. Каждый существенный факт имеет evidence-действие. Tap по связанной сущности сворачивает панель, центрирует камеру на новой ноде и обновляет досье. Локальная кнопка назад восстанавливает предыдущий выбор и камеру. Историческая динамика состояния показывается только при наличии минимум трёх сопоставимых оценок.
