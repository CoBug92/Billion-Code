...
В репозитории находятся каноническая документация и нативный технический spike. Generated Xcode-проект загружает source-backed bundled seed через actor-isolated `ContentStore` и показывает people-only проекцию кластера Илона Маска. Dataset предназначен только для локальной оценки дизайна: полноценный MVP и готовый к публикации редакционный контент ещё не реализованы.
...
Документация описывает желаемый MVP и решения, принятые 31 августа — 1 сентября 2026 года. People-only является временным presentation experiment и не меняет schema v1 или принятое решение о mixed evidence graph. Graph и `ContentStore` остаются `proposed` до performance/accessibility audit и fault injection. Лицензирование оценки состояния и портретов имеет статус `blocked for publication`; при отсутствии проверенного изображения UI показывает системный силуэт.
